...
do
   build/X86/gem5.opt configs/rely_backend/se.py --cmd=$BENCHMARK_PATH --cpu-type=AtomicSimpleCPU --fi-fpu-reliability=$FPU_RELIABILITY --fi-alu-reliability=$ALU_RELIABILITY --fi-function_name=$FUNCTION_NAME --fi-verbose> log 2> err_log
   if grep "$CORRECT" log
   then
        let "NUM_CORRECT+=1"
